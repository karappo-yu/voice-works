...
public class DsLite {
    ...
    private final static String BASE_URL = "https://www.dlsite.com/maniax/work/=/product_id/RJ";

    private final static String DYNAMIC_AJAX_URL = "https://www.dlsite.com/maniax-touch/product/info/ajax?product_id=RJ";

    private final static String IMAGE_ADDR = "https://img.dlsite.jp/modpub/images2/work/doujin/RJ{0}/RJ{1}_img_{2}.jpg";
    //0id2 1 rjCode 2type
    private final static String MAIN_IMAGE = "main";

    private final static String SAM_IMAGE = "sam";

    private final static String IMAG_NAME = "RJ{0}_{1}.jpg";
    ...
    public DsLite(SSYKConfigProperties properties) {
        this.timeout = properties.getTimeout();
        this.proxyAddr = properties.getProxyURL();
        this.proxyPort = properties.getProxyPort();
        this.imgPath = properties.getCoverPath();
    }
     ...
     * @param rjCode
     */
    public void getImage(String rjCode) {
    ...
    private void downloadImg(String rjCode, String url, String type) {
        File imagPath = new File(this.imgPath);
     ...
     * @return
     */
    public VoiceWorkDynamic getDynamicData(String id) {
        log.info("[RJ{}] 获取动态数据.....", id);
        String url = DYNAMIC_AJAX_URL + id;
        Document document = null;
        try {
            document = Jsoup.connect(url).proxy(this.proxyAddr, this.proxyPort)
    ...
    private VoiceWorkDynamic parseJsonData(String jsonData, String id) {
        ...
        try {
            ...
            List<Map<String,Object>> rateCountDetailMaps = (List<Map<String,Object>>) dataMap.get("rate_count_detail");

            List<RateCountDetail> rateCountDetails = new LinkedList<>();
            for (Map<String,Object> map1 : rateCountDetailMaps) {
            ...
            Integer reviewCount = Integer.parseInt(dataMap.get("review_count") + "");
     ...
     * @return
     */
    public VoiceWork getMetaData(String rjCode) {
        String url = BASE_URL + rjCode + ".html";
        Document document = null;
        try {
            log.info("[RJ{}] 获取音声静态数据.....", rjCode);
            document = Jsoup.connect(url)
                    .proxy(this.proxyAddr, this.proxyPort)
                    .ignoreHttpErrors(true)
                    .timeout(this.timeout).get();
        } catch (IOException e) {
    ...
    private VoiceWork getVoiceWork(Document document, String id) {
    ...
    private String getWorkFormat(Document document) {
    ...
    private String getImageAuthor(Document document) {
        Elements elements = document.getElementById("work_outline").child(0).children();
        for (Element element : elements) {
            String text = element.child(0).text();
    ...
    private Set<Cv> getCvs(Document document) {
        Elements elements = document.getElementById("work_outline").child(0).children();
    ...
    private Set<Tag> getTags(Document document) {
        Elements elements = document.getElementById("work_outline").child(0).children();
    ...
    private String getScenario(Document document) {
        Elements elements = document.getElementById("work_outline").child(0).children();
        for (Element element : elements) {
            String text = element.child(0).text();
    ...
    private String getRelease(Document document) {
        Elements elements = document.getElementById("work_outline").child(0).children();
        for (Element element : elements) {
            String text = element.child(0).text();
    ...
    private Circle getCircle(Document document) {
    ...
    private String getAgeRatings(Document document) {
        Elements elements = document.getElementById("work_outline").child(0).children();
        for (Element element : elements) {
            String text = element.child(0).text();
